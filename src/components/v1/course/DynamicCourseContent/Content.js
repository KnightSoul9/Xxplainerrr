...
const Content = ({ content, index, course, courseSlug, cTitle, sTitle }) => {
  const [isOpen, setIsOpen] = useState(true);
  const router = useRouter();

  // based on subModule.slug it will also work
  // const sortedSubmodules = [...content.subModules].sort((a, b) => {
  //   const aNumber = parseInt(a.slug.split("-")[0]);
  //   const bNumber = parseInt(b.slug.split("-")[0]);
  //   return aNumber - bNumber;
  // });

  const [showModal, setShowModal] = useState(false);

  const handleClick = (sortedSubModule) => {
    const hasLeadEmail = localStorage.getItem("leadInfo");
    const clickedUrl = `/learning/${courseSlug}/${content?.title}/${sortedSubModule?.slug}`;
    // console.log(clickedUrl, "hh");
    localStorage.setItem("clickedUrl", clickedUrl);
    if (hasLeadEmail) {
      router.push(
        `/learning/${courseSlug}/${content?.title}/${sortedSubModule?.slug}`
      );
    } else {
      setShowModal(true);
    }
          ...
          {getSortedSubmodules().map((sortedSubModule, index) => (
            ...
            >
              <div
                ...
                onClick={() => handleClick(sortedSubModule)}
              >
